...
#define FADE_CYCLE_SECONDS 4.0f
#define MIN_TINT 0.4f
...
SplashAnimation::Load(vrb::RenderContextPtr& aContext, const DeviceDelegatePtr& aDeviceDelegate) {
  ...
  vrb::TextureGLPtr texture = create->LoadTexture("splash.png");
...
SplashAnimation::Update(const vrb::Matrix& aHeadTransform) {
  ...
  //}
  float pos = fmod(m.time, FADE_CYCLE_SECONDS) / FADE_CYCLE_SECONDS;
  float coef = (pos < 0.5f? pos : 1.0f - pos) / 0.5f; // 0 -> 1 -> 0  
  float t = MIN_TINT + (1.0f - MIN_TINT) * coef;
  m.logo->SetTintColor(vrb::Color(t, t, t, 1.0f));
